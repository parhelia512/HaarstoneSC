        ...
        if (data.pointerDrag.CompareTag("Card"))
        {
            ...
            if (hero.currentMana >= 1)
            {
                if (card.card.tradeable)
                {
                    ...
                    GameObject.Find("GameplayManager").GetComponent<GameplayManager>().DrawCard(card.gameObject, data.pointerDrag.gameObject.transform.root.name == "Player" ? Turn.PLAYER : Turn.OPPONENT);
                    deck.ShuffleDeck();
                }
            }
        }

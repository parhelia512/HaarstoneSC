...
public class MinionCardEffects : MonoBehaviour
{
    ...
    {
        GameObject.Find("GameplayManager").GetComponent<GameplayManager>().DrawCard(minion.gameObject, minion.transform.root.name == "Player" ? Turn.PLAYER : Turn.OPPONENT);
    }
    ...
    public void Googlebot(List<GameObject> target, MinionScript minion)
    {
        ...
        foreach (CardSO card in deck.cards)
        {
            if (card.cardType == CardType.MINION)
            {
                ...
                {
                    GameObject.Find("GameplayManager").GetComponent<GameplayManager>().DrawCard(minion.gameObject, minion.transform.root.name == "Player" ? Turn.PLAYER : Turn.OPPONENT, card);
                    return;
                }
            }
        }
    ...
    public void Bananaman(List<GameObject> target, MinionScript minion)
    {
        HeroScript whoseTurn = minion.transform.root.Find("Hero").transform.Find("Canvas").transform.Find("Hero").GetComponent<HeroScript>();
        for (int i = 0; i < 3; i++)
        {
            whoseTurn.deck.cards.Add(minion.card.tokens[0]);
        }
        whoseTurn.deck.ShuffleDeck();
    }
    ...
    public void DaybreakDeceptor(List<GameObject> target, MinionScript minion)
    {
        GameObject.Find("GameplayManager").GetComponent<GameplayManager>().DrawCard(minion.gameObject, minion.transform.root.name == "Player" ? Turn.PLAYER : Turn.OPPONENT);
        if ((minion.transform.root.name == "Player" && GameObject.FindGameObjectsWithTag("Player Minion").Length == 1) || (minion.transform.root.name == "Opponent" && GameObject.FindGameObjectsWithTag("Opponent Minion").Length == 1)) GameObject.Find("GameplayManager").GetComponent<GameplayManager>().DrawCard(minion.gameObject, minion.transform.root.name == "Player" ? Turn.PLAYER : Turn.OPPONENT);
    }
    ...
    public void CityCrier(List<GameObject> target, MinionScript minion)
    {
        ...
        GameObject.Find("GameplayManager").GetComponent<GameplayManager>().DrawCard(minion.gameObject, minion.transform.root.name == "Player" ? Turn.PLAYER : Turn.OPPONENT, chosenCard);
    }
    ...
    public async void Yoggerz(List<GameObject> target, MinionScript minion)
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            List<CardSO> _minions = GameObject.Find("GameplayManager").GetComponent<GameplayManager>().allPlayableCards.Where(a => a.cardType == CardType.MINION).ToList();
